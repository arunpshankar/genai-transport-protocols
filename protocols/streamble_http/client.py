...
def check_server_health():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"{Fore.GREEN}✅ FastAPI HTTP streaming server is healthy!{Style.RESET_ALL}")
            ...
            print(f"{Fore.CYAN}   Framework: FastAPI + HTTP Streaming{Style.RESET_ALL}")
            return True
        else:
        ...
        print(f"{Fore.RED}❌ Cannot connect to FastAPI HTTP streaming server at {SERVER_URL}{Style.RESET_ALL}")
        print(f"{Fore.YELLOW}💡 Make sure the streamable HTTP server is running on port 8000{Style.RESET_ALL}")
...
def log_stream_start(message: str, session_info: str):
    ...
    print(f"{Fore.BLUE}│{Style.RESET_ALL} Framework: {Fore.MAGENTA}FastAPI + HTTP Streaming{Style.RESET_ALL}")
    print(f"{Fore.BLUE}│{Style.RESET_ALL} Transfer-Encoding: chunked")
    print(f"{Fore.BLUE}│{Style.RESET_ALL} Content-Type: application/x-ndjson")
...
def log_stream_complete(total_chunks: int, total_time: float, message_count: int):
    ...
    print(f"{Fore.GREEN}│{Style.RESET_ALL} Transfer-Encoding: chunked")
...
def print_help():
    ...
    print(f"{Fore.CYAN}  /demo{Style.RESET_ALL}     - Open HTTP streaming demo page in browser")
...
def get_server_stats():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"\n{Fore.CYAN}┌─ 🖥️  HTTP STREAMING SERVER STATISTICS ──────────────────────┐{Style.RESET_ALL}")
            ...
            print(f"{Fore.CYAN}│{Style.RESET_ALL} Framework: {Fore.MAGENTA}FastAPI + HTTP Streaming{Style.RESET_ALL}")
            print(f"{Fore.CYAN}└────────────────────────────────────────────────────────────┘{Style.RESET_ALL}")
        else:
...
def open_demo():
    ...
    try:
        ...
        print(f"{Fore.GREEN}📡 Opening HTTP streaming demo page in your browser...{Style.RESET_ALL}")
...
def send_message_stream(user_message: str):
    ...
    try:
        # Send POST request for HTTP stream
        ...
        if response.status_code == 200:
            ...
            # Process HTTP streaming response (NDJSON)
        ...
        session_stats['failed_requests'] += 1
        print(f"\n{Fore.RED}🔌 Connection failed. Is the FastAPI HTTP streaming server running?{Style.RESET_ALL}")
...
def main():
    ...
    if not check_server_health():
        print(f"\n{Fore.RED}Cannot connect to FastAPI HTTP streaming server. Exiting...{Style.RESET_ALL}")
        return
    
    print(f"\n{Fore.GREEN}🎉 Connected to FastAPI HTTP streaming server successfully!{Style.RESET_ALL}")
    ...
    print(f"{Fore.YELLOW}💡 Type '/demo' to open the web-based HTTP streaming demo{Style.RESET_ALL}")
    print(f"{Fore.YELLOW}💡 Type '/quit' to exit{Style.RESET_ALL}")
    print(f"\n{Fore.WHITE}{'═' * 60}{Style.RESET_ALL}")
    print(f"{Fore.CYAN}📡 HTTP chunked streaming enabled - responses will appear as they're generated!{Style.RESET_ALL}")
    ...
    while True:
        try:
            ...
            if user_message.lower() == '/quit':
                print(f"\n{Fore.YELLOW}👋 Thanks for chatting with FastAPI HTTP Streaming! Goodbye!{Style.RESET_ALL}")
                print_session_stats()
                break
            ...
            # Send regular message with HTTP streaming
