...
async def lifespan(app: FastAPI):
    # Startup
    print_banner()
    print(f"\n{Fore.GREEN}🚀 FastAPI SSE Multi-turn Chat Server starting up...{Style.RESET_ALL}")
    ...
    print(f"{Fore.YELLOW}🌊 SSE Demo: http://localhost:8000/demo{Style.RESET_ALL}")
    print(f"{Fore.YELLOW}🔧 Use Ctrl+C to stop the server{Style.RESET_ALL}\n")
    
    yield
    
    # Shutdown
    print(f"\n\n{Fore.YELLOW}👋 FastAPI SSE server shutting down gracefully...{Style.RESET_ALL}")
...
app = FastAPI(
    title="GenAI SSE Multi-turn Chat API",
    description="Beautiful FastAPI-based SSE multi-turn chat server with real-time streaming",
    version="2.0.0",
    lifespan=lifespan
)
...
║  Streaming: {Fore.GREEN}REAL-TIME{Fore.CYAN}{' ' * 40} ║
...
def log_stream_start(session_id: str, client_ip: str, message: str):
    ...
    print(f"\n{Fore.MAGENTA}┌─ 🌊 SSE STREAM STARTED [{timestamp}] ──────────────────────────┐{Style.RESET_ALL}")
...
def print_stats():
    ...
    print(f"\n{Fore.CYAN}┌─ 📊 SSE SERVER STATISTICS ──────────────────────────────────┐{Style.RESET_ALL}")
...
async def generate_chat_stream(chat_session: ChatSession, user_message: str, session_id: str, client_ip: str) -> AsyncGenerator[str, None]:
    ...
    try:
        ...
        yield json.dumps({'type': 'session_info', 'session_id': session_id, 'model': chat_session.model_id})
        
        # Add user message to session
        chat_session.add_message("user", user_message)
        
        # Send status update
        yield json.dumps({'type': 'status', 'message': 'Generating response...', 'context_messages': chat_session.get_message_count()})
...
async def chat_stream(request: ChatStreamRequest, http_request: Request):
    """Main SSE chat endpoint with streaming response"""
...
async def demo_page():
    """Simple demo page for testing SSE"""
    html_content = """
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
        <div class="container">
            <h1>🌊 SSE Multi-turn Chat Demo</h1>
        ...
        <script>
            ...
            // Initialize
            addMessage('🌊 SSE Chat Demo Ready! Type a message to start or click "New Session"', 'status');
        </script>
    </body>
    </html>
    """
...
async def root():
    ...
    return {
        "name": "GenAI SSE Multi-turn Chat API",
        "version": "2.0.0",
        "framework": "FastAPI + SSE",
        "model": MODEL_ID,
        "features": ["multi-turn conversations", "real-time streaming", "session management"],
        "endpoints": {
...
if __name__ == '__main__':
    try:
        ...
        print(f"\n{Fore.YELLOW}👋 SSE Server interrupted by user{Style.RESET_ALL}")